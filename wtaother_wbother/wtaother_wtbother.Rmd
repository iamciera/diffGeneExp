...
*wt* other A (tip) vs Marginal other in B (middle) region 
====================================================
...
```{r}
colnames(counts) 
wtaotherVSwtbother <- counts[,c(27:31,37:39)]
head(wtaotherVSwtbother)
colnames(wtaotherVSwtbother)
```

```{r}
group <- c(rep("wtaother", 5), rep("wtbother", 3))
d <- DGEList(counts=wtaotherVSwtbother,group=group)
```
...
DEtest <- exactTest(d,pair=c("wtaother","wtbother"))
head(DEtest$table)
...
```{r}
write.table(results.all.annotated,"wtaother_wtbother_DE_all.txt",sep="\t",row.names=F)
write.table(results.sig.annotated,"wtaother_wtbother_DE.txt",sep="\t",row.names=F)
```

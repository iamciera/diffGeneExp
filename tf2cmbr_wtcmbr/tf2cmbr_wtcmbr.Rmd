...
```{r}
group <- c(rep(region1, 4), rep(region2, 4))
...
```{r}
write.table(results.all.annotated,"tf2cmbr_wtcmbr_DE_all.txt",sep="\t",row.names=F)
write.table(results.sig.annotated,"tf2cmbr_wtcmbr_DE.txt",sep="\t",row.names=F)
```

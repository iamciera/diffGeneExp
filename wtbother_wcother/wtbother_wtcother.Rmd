...
WT other B (middle) vs WT in other C (base) region 
====================================================
...
```{r}
colnames(counts)
wtbotherVSwtcother <- counts[,c(36:39,46:48)]
head(wtbotherVSwtcother)
colnames(wtbotherVSwtcother)
```

```{r}
group <- c(rep("wtbother", 4), rep("wtcother", 3))
d <- DGEList(counts=wtbotherVSwtcother,group=group)
```
...
DEtest <- exactTest(d,pair=c("wtbother","wtcother"))
head(DEtest$table)
...
```{r}
write.table(results.all.annotated,"wtbother_wtcother_DE_all.txt",sep="\t",row.names=F)
write.table(results.sig.annotated,"wtbother_wtcother_DE.txt",sep="\t",row.names=F)
```

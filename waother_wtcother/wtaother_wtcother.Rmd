...
WT Other A (tip) vs WT Other in C (base) region 
====================================================
...
```{r}
colnames(counts)
wtaotherVSwtcother <- counts[,c(27:31, 46:48)]
head(wtaotherVSwtcother)
colnames(wtaotherVSwtcother)
```

```{r}
group <- c(rep("wtaother", 5), rep("wtcother", 3))
d <- DGEList(counts=wtaotherVSwtcother,group=group)
```
...
DEtest <- exactTest(d,pair=c("wtaother","wtcother"))
head(DEtest$table)
...
```{r}
write.table(results.all.annotated,"wtaother_wtcother_DE_all.txt",sep="\t",row.names=F)
write.table(results.sig.annotated,"wtaother_wtcother_DE.txt",sep="\t",row.names=F)
```
